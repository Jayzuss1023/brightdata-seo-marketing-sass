...
export function getSpinnerColor(status: string): string {
  const statusConfig = {
    ...
    running: "text-blue-600 dark:text-yellow-400",
...
export function getStatusMessage(status: string): string {
  switch (status) {
    case "pending":
      return "You report is queued and will start processing shortly.";
    case "running":
