...
export function SourceDistributionChart({
  ...
  // Process the source type data from the SEO report into a format suitable for the Pie Chart
  // Extracts source types and their counts from seoReport.inventory.source_types
  // Assign colors from a predefined palette to each source type
  ...
  const sourceTypeEntries = (() => {
    const st = seoReport?.inventory?.source_types as Record<
      string,
  ...
  return <div>SourceDistributionChart</div>;
}
